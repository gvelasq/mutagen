...
#'
#' @returns A binary integer vector indicating whether all supplied values were matched with an integer 1 (`1L`), otherwise it returns an integer 0 (`0L`).
#'
#' @examples
...
#' b
#' @export
gen_rowall <- function(data, cols, values) {
  ...
  x <- purrr::pmap_int(
    data,
    purrr::in_parallel(\(...) all(list(...) %in% values), values = values)
  )
  x
}
